...
if [ -z $1 ]; then
  ...
  if [ "$SESSION_EXISTS" = "" ]; then
    ...
    if [ -z $2 ]; then
      tmux send-keys -t 'nmap' "nmap -sV -oN nmap "
    else
      tmux send-keys -t 'nmap' "nmap -sV -oN nmap $2" Enter
    fi
    ...
    if [ -z $2 ]; then
      tmux send-keys -t 'nmap.right' "gobuster dir -e -w $W_COMMON -x php,html,js,css,cgi,txt,sh,py,rb -u http://"
    else
      tmux send-keys -t 'nmap.right' "gobuster dir -e -w $W_COMMON -x php,html,js,css,cgi,txt,sh,py,rb -u http://$2" Enter
    fi
